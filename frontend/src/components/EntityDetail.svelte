<script lang="ts">
  ...
  $effect(() => {
    void entityId;
    loading = true; error = null; ctx = null;
    api.entityContext(entityId)
      .then((c) => { ctx = c; })
...
<div class="detail-head">
  <div class="detail-title">{ctx?.entity.name ?? (loading ? "Loading…" : `Entity #${entityId}`)}</div>
...
<div class="detail-body">
  ...
  {:else if ctx}
    ...
    <dl class="deflist">
      <dt>id</dt><dd>{ctx.entity.id}</dd>
      <dt>type</dt><dd class="plain">{ctx.entity.type}</dd>
      <dt>name</dt><dd class="plain">{ctx.entity.name}</dd>
      {#if ctx.entity.description}<dt>summary</dt><dd class="plain">{ctx.entity.description}</dd>{/if}
    </dl>
    ...
    {#if ctx.related_entities.length}
      ...
      <div class="col" style="gap:6px">
        {#each ctx.related_entities as r}
          <button class="hover-tile row-gap" type="button" style="cursor:pointer;text-align:left"
      ...
      <div class="col" style="gap:6px">
        {#each ctx.evidence as ev}
          <div class="hover-tile">
            <div class="dim mono" style="font-size:10.5px;letter-spacing:0.1em;text-transform:uppercase">
              {ev.source_type}{ev.source_id ? ` · ${ev.source_id}` : ""}
            </div>
            {#if ev.quote}<div style="margin-top:4px;font-size:12.5px;color:var(--text-2);line-height:1.5">{ev.quote}</div>{/if}
          </div>
        {/each}
      </div>
    {/if}
