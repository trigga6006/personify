...
<div class="page-head">
  ...
  <h1>Search</h1>
  <p class="lede">Full-text or semantic, scoped or vault-wide. Click any result to open the detail panel.</p>
</div>

<div class="toolbar">
  <input type="search" class="grow" bind:value={q} placeholder="Search the vault…"
  ...
  <div class="seg" role="tablist">
    <button type="button" role="tab" class:on={mode === "text"} aria-selected={mode === "text"} onclick={() => (mode = "text")}>Text</button>
    <button type="button" role="tab" class:on={mode === "semantic"} aria-selected={mode === "semantic"} onclick={() => (mode = "semantic")}>Semantic</button>
  </div>
...
{#if loading}
  <div class="hit-list">
    {#each Array(5) as _}<div class="hit"><Skeleton width="60px" height="20px" /><div class="body"><Skeleton width="60%" /><div style="height:6px"></div><Skeleton width="90%" height="12px" /></div></div>{/each}
  </div>
{:else if error}
  <div class="error-box">{error}</div>
{:else if !ran}
  <Empty icon="⌕" title="Type a query, press Enter" sub="Toggle Semantic to search by meaning instead of literal words. Restrict by source to narrow the scope." />
{:else if hits.length === 0}
  <Empty icon="⌕" title="No results" sub={`No matches for “${q}” ${source ? `in ${source}` : ""}.`} />
{:else}
  <div class="row-gap" style="margin-bottom:14px;font-size:12px;color:var(--text-3)">
    <span class="mono" style="color:var(--text-2)">{hits.length}</span> result{hits.length === 1 ? "" : "s"}
